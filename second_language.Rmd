...
# Get existing tweets
for(i in list.files(pattern = "all_tweets")){
  load(i)  
}

# combine
x <- dplyr::bind_rows(ls(pattern = "all_tweets"))
...
# Clean up dups (list columns seem to be throwing off distinct (and maybe unique))
# dups result from my inelegant method of saving/reloading tweets...
...
assign(paste0("all_tweets_",lubridate::today()), all_tweets)
save(list = paste0("all_tweets_",lubridate::today()), 
     file = paste0("all_tweets_",lubridate::today(),".Rda"))
```
...
First I wanted to tag each of the tweets with a language (or languages)

```
languages <- c("r","c++","javascript","python","rust","go","golang","ruby",
"sql","julia",)