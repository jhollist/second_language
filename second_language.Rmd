...
languages <- c("r","c\\+\\+","c\\/c\\+\\+", "c", "javascript","python","rust","go","golang","ruby",
"sql","julia", "tensorflow", "swift", "spark", "scratch","sas","rails","perl", "keras","julialang","js","java","julia's","html","hive","haskell","golang","fortran","ethereum","d3","css","c#")
languages<- stringr::str_c("\\b",languages, "\\b")

all_replies<-all_replies %>% 
  mutate(clean_text = gsub("^#$!$?$.","",tolower(text)))

x<-vector("numeric", length = length(languages))
for(i in seq_along(languages)){
x[i]<-all_replies$clean_text %>% 
  str_match(languages[i]) %>%
  data.frame(stringsAsFactors = FALSE) %>%
  mutate(logical = is.na(.) == FALSE) %>%
  pull(logical) %>%
  sum()
}
